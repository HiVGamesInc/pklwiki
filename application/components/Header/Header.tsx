...
export default function Header() {
  ...
  return (
    <header className={`w-full bg-bgPrimary shadow-md border-b ${darkMode ? 'border-b-gray-800' : 'border-b-gray-100'}`}>
      <span className="flex items-center justify-between w-full max-w-screen-xl mx-auto h-16 p-4">
        <img src="https://placehold.co/200x50.png" alt="Logo" className="h-full"/>
        <div className="flex items-center relative">
          <Input value='' />
          <button className="h-4 block text-dBlack absolute top-1/2 right-0 translate-x-[-50%] translate-y-[-50%]"><Icon icon={MagnifyingGlassIcon} /></button>
        </div>
